...
/* Here is an extreme implementation of GRSerializable that takes very unconventional objects (Class, NSPredicate and NSSortDescriptor) and serializes them. Your own implementation of GRSerializable should not be remotely this bad, but I guess this serves as a good example. All we're doing is replacing each Class, NSPredicate and NSSortDescriptor property with a string representation of itself in dictionaryRepresentationWithContext: and then using that representation in initWithDictionaryRepresentation to recreate the actual properties.
 */

static NSString * const GRCollectionPropertyObjects         = @"objects";
static NSString * const GRCollectionPropertyClasses         = @"classes";
static NSString * const GRCollectionPropertyPredicate       = @"predicate";
static NSString * const GRCollectionPropertySortDescriptors = @"sortDescriptors";

static NSString * const GRCollectionSortDescriptorSeparator     = @"><";
static NSString * const GRCollectionSortDescriptorAscendingKey  = @"ASC";
static NSString * const GRCollectionSortDescriptorDescendingKey = @"DESC";

+(void)load
{
    // When this class is added to the runtime, teach GRSerialization to serialize NSPredicate and NSSortDescriptor objects
    [GRSerialization learnConversionForClass:[NSPredicate class] converter:^id(NSPredicate *predicate, NSString *format){
        return predicate ? predicate.predicateFormat : [NSPredicate predicateWithFormat:format];
    }];
    [GRSerialization learnConversionForClass:[NSPredicate class] converter:^id(NSSortDescriptor *sortDescriptor, NSString *format){
        if (sortDescriptor)
        {
            // Create a string representation for the sortDescriptor
            NSMutableString *sortDescriptorRepresentation = [NSMutableString string];

            // Append the key
            [sortDescriptorRepresentation appendString:sortDescriptor.key];
            [sortDescriptorRepresentation appendString:GRCollectionSortDescriptorSeparator];

            // Append ascending
            [sortDescriptorRepresentation appendString:sortDescriptor.ascending ? GRCollectionSortDescriptorAscendingKey : GRCollectionSortDescriptorDescendingKey];
            [sortDescriptorRepresentation appendString:GRCollectionSortDescriptorSeparator];

            // Append selector if neccesary
            if (sortDescriptor.selector)
                [sortDescriptorRepresentation appendString:NSStringFromSelector(sortDescriptor.selector)];

            return sortDescriptorRepresentation;
        }
        else
        {
            // Get components: 0. key 1. ascending (2. selector)
            NSArray *components = [format componentsSeparatedByString:GRCollectionSortDescriptorSeparator];

            // Add sort descriptor with/without selector
            if ([components count] == 2)
                return [NSSortDescriptor sortDescriptorWithKey:components[0] ascending:[components[1] isEqualToString:GRCollectionSortDescriptorAscendingKey]];
            else if ([components count] == 3)
                return [NSSortDescriptor sortDescriptorWithKey:components[0] ascending:[components[1] isEqualToString:GRCollectionSortDescriptorAscendingKey] selector:NSSelectorFromString(components[2])];
            else
                return nil;
        }
    }];
...
{
    // Create a collection with the serialized parameters
    NSMutableArray *parameters = [NSMutableArray array];
    [parameters addObject:[dictionaryRepresentation valueForKey:GRCollectionPropertyClasses]];
    [parameters addObjectsFromArray:[dictionaryRepresentation valueForKey:GRCollectionPropertySortDescriptors]];
    NSPredicate *predicate = [dictionaryRepresentation valueForKey:GRCollectionPropertyPredicate];
    if (predicate) [parameters addObject:predicate];

    return [GRCollection collectionWithParameters:[parameters copy]];
}
